...
| ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--boltz_random_seed`    | `null`   | Boltz inference is stochastic. Fixing a numerical random seed helps make predictions reproducible between runs. In Boltz mode this is passed to the native `--seed` flag and overwrites the global `--random_seed` value when both are set. |
